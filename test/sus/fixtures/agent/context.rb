...
describe Sus::Fixtures::Agent::Context do
	...
	with "sus context" do
		...
		it "correctly implement an array expectation" do
			with_agent_context(context_path, model: "mixtral:latest") do |session|
				response = session.ask("How do I write a Sus expectation for checking if an Array `array` includes an `item`?")
		...
		it "correctly implement a hash expectation" do
			...
			with_agent_context(context_path, model: "mixtral:latest") do |session|
				response = session.ask("How do I write a Sus expectation for checking if a Hash `hash` includes a key `:key`?")
